...
Tie::TransactHash to pakiet udostępniający możliwość modyfikowania
innych haszy w transakcjach. Transakcja to grupa zmian, które albo są
wykonywanie wszystkie, albo żadna. Przy pracy z normalnymi haszami
perlowymi lub plikiem DBM poindeksowanym haszem, zaletą jest to, że
oryginalny hasz pozostaje nietknięty podczas transakcji, więc jego
kolejność (kolejność, w jakiej zwracają wartości funkcje each(),
keys() i values()) jest zachowana - zmainy mogą być robione w
